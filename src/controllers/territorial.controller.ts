...
export async function getRegion(req: Request, res: Response): Promise<Response> {
    ...
    const conn = await connect();
    const region = await conn.query(`select * from region where id_region = ?`, [id]);
...
export async function getComuna(req: Request, res: Response): Promise<Response> {
    const id = req.params.comunaId;
    const conn = await connect();
    const comuna = await conn.query(`select * WHERE id_comuna = ?`, [id]);
